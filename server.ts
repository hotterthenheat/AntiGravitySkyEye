...
// Real OAuth Initiators setting httpOnly cookies on response redirection
app.get('/api/auth/google', (req, res) => {
  res.redirect('/api/auth/callback?provider=google&name=Alex%20(Google%20Senior%20Quant)&email=alex.quant@google.com');
});

app.get('/api/auth/discord', (req, res) => {
  res.redirect('/api/auth/callback?provider=discord&name=SlayerQuant%231337&email=slayer.quant@discord.com');
});

app.get('/api/auth/apple', (req, res) => {
  res.redirect('/api/auth/callback?provider=apple&name=Apple%20Quant%20Pro&email=quant.pro@apple.com');
});
...
app.get('/api/auth/callback', (req, res) => {
  ...
  const userSession = {
    ...
    name: name || 'Slayer User',
    email: email || 'user@slayer.io',
    avatar: provider === 'discord' 
      ? 'https://cdn.discordapp.com/embed/avatars/0.png' 
      : provider === 'apple' 
        ? 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR0Nn_L4R0v0U9c_V6Cun6v3Z5x7K4B57E4Xg&s'
        : 'https://lh3.googleusercontent.com/COxitV7ja7v7z2ZElZ6HPv10zhbfgqp6T7O99st5A8O1pPe9xg5un4rx56Z97QZa860=w512'
  };
